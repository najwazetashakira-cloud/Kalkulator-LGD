...
with col_unit1:
    time_unit = st.radio(
        ...
        help=(
            ...
            "Bulan: cocok untuk penyelesaian cepat 3–60 bulan. "
...
p_nothing = max(round(1.0 - p1 - p2, 10), 0.0)
...
if full_claim < ead:
    st.markdown("""
    <div class="card card-amber"><span class="badge badge-amber">Perhatian</span>
    <p style="margin:0">Total tagihan penuh lebih kecil dari EAD (pokok). Lazimnya tagihan penuh
    ≥ pokok karena mencakup bunga dan denda. Pastikan nilai ini sudah benar.</p>
    </div>""", unsafe_allow_html=True)
...
# ============================================================
# HELPER FUNCTIONS
# Single source of truth untuk semua formula kritis.
# ============================================================

def compute_bep_lower(full_claim, recovery_now, t, r, collection_cost, cost_timing):
    """
    Breakeven p₁ ketika p₂ = 1 − p₁ (skenario paling optimis: tidak ada yang default total).

    Derivasi analitik dari PV_A(p₁, p₂=1−p₁) = PV_B = recovery_now.
    Ini adalah single source of truth — dipakai oleh calculate_values() dan Tab 4,
    sehingga kedua tempat selalu konsisten.

    Return None jika full_claim ≤ recovery_now (tidak ada solusi).
    """
    if full_claim <= recovery_now:
        return None
    df = (1 + r) ** t
    if cost_timing == "Akhir periode recovery":
        val = (recovery_now * (df - 1) + collection_cost) / (full_claim - recovery_now)
    else:
        val = (recovery_now * (df - 1) + collection_cost * df) / (full_claim - recovery_now)
    return max(val, 0.0)


def calculate_values(ead, full_claim, recovery_now, p1, p2, t, r, collection_cost, cost_timing):
    """
    Hitung semua metrik utama kalkulator.
    ...
    Return dict mencakup breakeven_p1 (untuk p₂ yang sedang dipilih)
    dan bep_lower (untuk p₂ = 1−p₁, via compute_bep_lower).
    """
    df            = (1 + r) ** t
    p_nothing     = max(1.0 - p1 - p2, 0.0)
    exp_nominal   = p1 * full_claim + p2 * recovery_now

    if cost_timing == "Akhir periode recovery":
        pv_a         = (exp_nominal - collection_cost) / df
        breakeven_p1 = (recovery_now * df + collection_cost - p2 * recovery_now) / full_claim
    else:
        pv_a         = (exp_nominal / df) - collection_cost
    ...
    # bep_lower: via compute_bep_lower — satu sumber kebenaran
    _bep_lower = compute_bep_lower(full_claim, recovery_now, t, r, collection_cost, cost_timing)

    return {
        "df": df,
        "expected_nominal": exp_nominal,
        "p_nothing": p_nothing,
        "pv_a": pv_a, "pv_b": pv_b,
        ...
        "bep_lower": _bep_lower if _bep_lower is not None else breakeven_p1,
        "diff": pv_a - pv_b,
    }


def build_export_csv(params, res, satuan):
    """Bangun DataFrame ringkasan untuk diekspor sebagai CSV."""
    t_display = f"{int(params['t'])}" if params['t'] == int(params['t']) else f"{params['t']:.1f}"
    rec = "Opsi A" if res["diff"] > 0.1 else ("Opsi B" if res["diff"] < -0.1 else "Setara")
    rows = {
        "Item": [
            "── PARAMETER INPUT ──",
            "EAD / pokok saat default (juta Rp)",
            "Total tagihan penuh (juta Rp)",
            "Recovery sekarang - Opsi B (juta Rp)",
            "Satuan waktu",
            f"Waktu tunggu t",
            "Discount rate r",
            "p1 — bayar penuh",
            "p2 — bayar pokok terlambat",
            "p0 — tidak bayar sama sekali",
            "Biaya collection (juta Rp)",
            "Timing biaya collection",
            "── HASIL UTAMA ──",
            "PV Recovery — Opsi A (juta Rp)",
            "PV Recovery — Opsi B (juta Rp)",
            "Selisih PV (A − B) (juta Rp)",
            "LGD raw — Opsi A",
            "LGD raw — Opsi B",
            "LGD floor 0% — Opsi A",
            "LGD floor 0% — Opsi B",
            "Rekomendasi",
            "── BREAKEVEN ──",
            f"Breakeven p1 min (p2 tetap = {params['p2']:.0%})",
            "Breakeven p1 min (p2 optimistis = 1−p1)",
        ],
        "Nilai": [
            "",
            f"{params['ead']:.2f}",
            f"{params['full_claim']:.2f}",
            f"{params['recovery_now']:.2f}",
            params["time_unit"],
            f"{t_display} {satuan}",
            f"{params['r']*100:.4f}% per {satuan}",
            f"{params['p1']:.2%}",
            f"{params['p2']:.2%}",
            f"{params['p_nothing']:.2%}",
            f"{params['collection_cost']:.2f}",
            params["cost_timing"],
            "",
            f"{res['pv_a']:.2f}",
            f"{res['pv_b']:.2f}",
            f"{res['diff']:.2f}",
            f"{res['lgd_a_raw']*100:.2f}%",
            f"{res['lgd_b_raw']*100:.2f}%",
            f"{res['lgd_a_floored']*100:.2f}%",
            f"{res['lgd_b_floored']*100:.2f}%",
            rec,
            "",
            f"{res['breakeven_p1']*100:.2f}%",
            f"{res['bep_lower']*100:.2f}%",
        ],
    }
...
res = calculate_values(
    ead, full_claim, recovery_now, p1, p2, t, r, collection_cost, cost_timing
)
...
# ============================================================
# KONFIGURASI SENSITIVITY & CHART — DINAMIS berdasarkan t aktual
# ============================================================
# FIX: t_vals, t_grid_bep, t_lim_bep dibangun dari t aktual sehingga:
#   (a) parameter aktif selalu masuk kolom sensitivity table → ★ muncul
#   (b) breakeven chart tidak terpotong saat t > batas lama
#
# Strategi: ambil breakpoints baku ≤ max(t, minimum_range),
# lalu tambahkan t aktual jika belum ada.

if time_unit == "Bulan":
    t_int       = int(t)
    _base_t     = [3, 6, 9, 12, 18, 24, 36, 48, 60]
    _upper      = max(t_int, 24)                        # tampilkan minimal hingga 24 bln
    t_vals      = sorted(set([v for v in _base_t if v <= _upper] + [t_int]))
    t_cols      = [f"{x} bln" for x in t_vals]

    t_max_bep   = max(float(t), 24.0)                   # chart minimal sampai 24 bln
    t_grid_bep  = np.arange(0.0, t_max_bep + 1.0, 1.0) # step 1 bulan
    t_lim_bep   = t_max_bep
    xlabel_bep  = "Waktu tunggu recovery (bulan)"
    r_label     = f"r_m = {r*100:.2f}%/bln"
    _t_tol      = 1.5                                    # toleransi ★ (bulan)

else:
    _base_t     = [0.5, 1.0, 2.0, 3.0, 5.0, 7.0, 10.0]
    _upper      = max(t, 5.0)                            # tampilkan minimal hingga 5 thn
    t_vals      = sorted(set([v for v in _base_t if v <= _upper] + [t]))
    t_cols      = [
        f"{x:.1f} thn" if x != int(x) else f"{int(x)} thn"
        for x in t_vals
    ]

    t_max_bep   = max(t, 5.0)                            # chart minimal sampai 5 thn
    t_grid_bep  = np.arange(0.0, t_max_bep + 0.25, 0.25)
    t_lim_bep   = t_max_bep
    xlabel_bep  = "Waktu tunggu recovery (tahun)"
    r_label     = f"r = {r:.0%}/thn"
    _t_tol      = 0.3                                    # toleransi ★ (tahun)
...
with tab1:
    ...
    # ── Breakeven range — menggunakan bep_lower dari res (via compute_bep_lower) ──
    bep_upper = calculate_values(
        ead, full_claim, recovery_now, p1, 0.0, t, r, collection_cost, cost_timing
    )["breakeven_p1"]
    ...
    if bep_hi_pct > 100:
        ...
        • Kalau p₂ besar → breakeven serendah <strong>{bep_lo_pct:.1f}%</strong>
        → <span style="color:#065f46;font-weight:600">Opsi A layak</span><br>
        • Kalau p₂ kecil atau nol → breakeven bisa sampai <strong>{bep_hi_pct:.1f}%</strong>
    ...
    csv_data = build_export_csv(
        {
            "ead": ead, "full_claim": full_claim, "recovery_now": recovery_now,
            "time_unit": time_unit, "t": t, "r": r,
            "p1": p1, "p2": p2, "p_nothing": p_nothing,
            "collection_cost": collection_cost, "cost_timing": cost_timing,
        },
        res, satuan
    )
    st.download_button(
        ...
        data=csv_data,
        file_name="workout_lgd_hasil.csv",
        mime="text/csv",
        help="Ekspor parameter input dan semua hasil utama dalam satu file CSV. "
             "Cocok untuk dokumentasi komite kredit."
    )
...
with tab2:
    ...
    with col_r3:
        st.metric("Bunga reinvestasi",
                  ...
                  delta=f"+{interest_earned/recovery_now*100:.1f}% dari dana awal"
                  if recovery_now > 0 else None)

    st.markdown("<hr class='divider'>", unsafe_allow_html=True)

    # FIX: header kolom menyesuaikan format t (integer vs float)
    t_display = f"{int(t)}" if t == int(t) else f"{t:.1f}"
    col_fv_label = f"Nilai pada akhir {satuan} ke-{t_display}"
    ...
    st.subheader(f"Perbandingan Future Value pada Akhir Periode ({t_display} {satuan})")
...
with tab3:
    ...
    t_display_tex = f"{int(t)}" if t == int(t) else f"{t:.1f}"
    ...
    if time_unit == "Bulan":
        ...
        """, unsafe_allow_html=True)
    st.latex(
        f"(1+{r_sym})^{{{t_sym}}} = (1+{r*100:.4f}\\%)^{{{t_display_tex}}} = {df_factor:.6f}"
    )
    ...
    if cost_timing == "Akhir periode recovery":
        st.markdown("### C. Present Value Recovery — Opsi A (biaya di akhir periode)")
        st.latex(
        ...
        st.markdown("### C. Present Value Recovery — Opsi A (biaya sekarang)")
        st.latex(
    ...
    st.markdown("### E. Breakeven p₁ — Batas Bawah (p₂ = 1 − p₁, paling optimis)")
    if cost_timing == "Akhir periode recovery":
        st.latex(
            r"p_{1,\min}^{\text{lower}} = \frac{R_{\text{now}} \cdot \left[(1+" + r_sym + r")^{" + t_sym + r"} - 1\right] + C}{R_{\text{full}} - R_{\text{now}}}"
            f" = {bep_lower*100:.2f}\\%"
        )
    else:
        st.latex(
            r"p_{1,\min}^{\text{lower}} = \frac{R_{\text{now}} \cdot \left[(1+" + r_sym + r")^{" + t_sym + r"} - 1\right] + C \cdot (1+" + r_sym + r")^{" + t_sym + r"}}{R_{\text{full}} - R_{\text{now}}}"
            f" = {bep_lower*100:.2f}\\%"
        )

    st.markdown("### F. Present Value Recovery — Opsi B")
    ...
    st.markdown("### H. Biaya Tersembunyi Skenario p₂")
    st.latex(
    ...
    st.markdown("### I. Future Value Opsi B — Reinvestasi")
    st.latex(
        r"FV_B = R_{\text{now}} \times (1+" + r_lend_sym + r")^{" + t_sym + r"}"
        f" = {recovery_now:,.2f} \\times (1 + {lending_rate*100:.4f}\\%)^{{{t_display_tex}}}"
...
with tab4:
    ...
    # FIX: t_vals, t_cols, t_grid_bep, t_lim_bep sudah dihitung secara dinamis
    # di blok konfigurasi sebelum tabs — tidak lagi hardcoded.
    ...
    for p1_i in p1_vals:
        ...
        for t_i in t_vals:
            ri = calculate_values(
                ead, full_claim, recovery_now, p1_i, p2, float(t_i), r, collection_cost, cost_timing
            )
            d  = ri["diff"]
            mk = " ★" if abs(p1_i - p1) < 0.06 and abs(float(t_i) - t) < _t_tol else ""
    ...
    for r_i in r_vals:
        ...
        for t_i in t_vals:
            ri = calculate_values(
                ead, full_claim, recovery_now, p1, p2, float(t_i), r_i, collection_cost, cost_timing
            )
            d  = ri["diff"]
            mk = " ★" if abs(r_i - r) < r_tol and abs(float(t_i) - t) < _t_tol else ""
            if d > 0.1:    row.append(f"A (+{d:,.2f}){mk}")
            elif d < -0.1: row.append(f"B ({d:,.2f}){mk}")
            else:           row.append(f"≈ setara{mk}")
        rows_b.append(row)
    ...
    # ── Tabel C: p₂ vs t (baru) ──
    st.subheader(f"C. p₂ vs Waktu Tunggu  (p₁ = {p1:.0%} tetap, {r_label})")
    st.markdown("""
    <div class="card card-blue"><p style="margin:0">
    p₂ adalah variabel paling sulit diestimasi di lapangan.
    Tabel ini menunjukkan seberapa besar p₂ mempengaruhi keputusan. <strong>★ = parameter aktif.</strong>
    </p></div>
    """, unsafe_allow_html=True)

    p2_vals = [0.0, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, max(round(1.0 - p1, 2), 0.0)]
    p2_vals = sorted(set(p2_vals))
    rows_c  = []
    for p2_i in p2_vals:
        if p2_i > round(1.0 - p1, 2) + 0.001:
            continue                                    # lewati p2 yang melebihi 1-p1
        row = []
        for t_i in t_vals:
            ri = calculate_values(
                ead, full_claim, recovery_now, p1, p2_i, float(t_i), r, collection_cost, cost_timing
            )
            d  = ri["diff"]
            mk = " ★" if abs(p2_i - p2) < 0.06 and abs(float(t_i) - t) < _t_tol else ""
            if d > 0.1:    row.append(f"A (+{d:,.2f}){mk}")
            elif d < -0.1: row.append(f"B ({d:,.2f}){mk}")
            else:           row.append(f"≈ setara{mk}")
        rows_c.append((p2_i, row))

    df_sc = pd.DataFrame(
        [r for _, r in rows_c],
        index=[f"p₂ = {p:.0%}" for p, _ in rows_c],
        columns=t_cols
    )
    df_sc.index.name = "p₂ (bayar pokok terlambat)"
    st.dataframe(df_sc, use_container_width=True)

    # ── Breakeven Chart — FIX: range dinamis, bep_lower via compute_bep_lower ──
    st.subheader("D. Breakeven Chart — Range p₁ Minimum vs Waktu Tunggu")
    st.markdown("""
    <div class="card card-blue"><p style="margin:0">
    <strong>Band abu-abu</strong> = range breakeven p₁ untuk semua kemungkinan p₂.<br>
    Batas bawah: p₂ = 1−p₁ (tidak ada yang tidak bayar — paling optimis).<br>
    Batas atas: p₂ = 0% (paling konservatif).<br>
    p₁ aktual <strong>di atas band</strong>: Opsi A layak di semua skenario. |
    <strong>di dalam band</strong>: tergantung asumsi p₂. |
    <strong>di bawah band</strong>: Opsi B lebih rasional.
    </p></div>
    """, unsafe_allow_html=True)

    # FIX: bep_upper_arr melalui calculate_values (sudah selalu benar)
    bep_upper_arr = np.array([
        calculate_values(
            ead, full_claim, recovery_now, p1, 0.0, t_i, r, collection_cost, cost_timing
        )["breakeven_p1"]
        for t_i in t_grid_bep
    ])

    # FIX: bep_lower_arr melalui compute_bep_lower — satu sumber kebenaran yang sama
    # dengan yang dipakai di Tab 1, tidak ada risiko inkonsistensi
    bep_lower_arr = np.array([
        compute_bep_lower(full_claim, recovery_now, t_i, r, collection_cost, cost_timing) or 0.0
        for t_i in t_grid_bep
    ])
    ...
    # FIX: titik parameter aktif selalu ada karena t_grid_bep mencakup t aktual
    idx_t = np.argmin(np.abs(t_grid_bep - t))
    ax2.scatter([t], [(bep_upper_disp[idx_t] + bep_lower_disp[idx_t]) / 2],
                s=90, color='#6b7280', zorder=6,
                label=f'Parameter aktif (t={int(t)} {satuan})')
...
with tab5:
    ...
    notes = [
        ...
        ("badge-blue", "2. Satuan waktu dan konversi discount rate",
         ...
         "dan <strong>bulanan</strong> (cocok untuk penyelesaian cepat 3–60 bulan). "
         ...
         "dan <strong>Efektif (^1/12−1)</strong> — cocok untuk akad EIR/XIRR. "
         "Untuk r = 12%/thn: nominal → 1,0000%/bln, efektif → 0,9489%/bln."),

        ("badge-blue", "3. Single source of truth untuk formula breakeven",
         "Formula breakeven batas bawah (p₂ = 1−p₁) dipusatkan dalam fungsi "
         "<code>compute_bep_lower()</code> yang dipanggil oleh <code>calculate_values()</code> "
         "dan oleh chart di Tab 6. Dengan begitu, kedua tempat selalu menggunakan logika yang sama "
         "dan tidak bisa keluar sinkron jika kode diubah."),

        ("badge-blue", "4. LGD dihitung terhadap EAD",
         "Konsisten dengan Basel II §460. LGD raw bisa negatif jika PV recovery > EAD — "
         "gunakan floor 0% untuk keperluan konservatif."),

        ("badge-blue", "5. Pemilihan discount rate",
        ...
        ("badge-blue", "6. Reinvestment analysis — basis future value",
        ...
        ("badge-purple", "7. Risiko moral hazard",
        ...
        ("badge-amber", "8. Batasan penggunaan",
